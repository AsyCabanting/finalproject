public class subjectDB : HTTPRequestHandler {
    ...
    public bool configure(File appdir, HashTable settings) {
        ...
        db = SQLiteDatabase.for_file(appdir.entry("subjectdb.sqlite"), true, get_logger());
		...
		if(db.table_exists("mysub") == false) {
            ...
            if(db.create_table("mysub", columns) == false) {
                log_error("Failed to create database table.");
                return(false);
            }
           
			insert_record("Project","80/100");
            insert_record("Quiz","28/30");
        }
    ...
    public void get(HTTPRequest req) {
        var sql = "Select * from mysub;"; //The sql query to execute
   ...
   bool insert_record(String data String data2) {
        var sql = "INSERT INTO mysub ( subject,score ) VALUES ( ?,? );";
